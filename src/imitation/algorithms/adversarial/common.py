    ...
    def _make_disc_train_batches(
        ...
        *,
        ...
        if self.device == 'cpu':
            ...
            for field in expert_samples:
                if isinstance(expert_samples[field], list):
                    if isinstance(expert_samples[field][0], th.Tensor):  # If it's a PyTorch tensor
                    ...
                    for field in list(expert_samples.keys()):
                        if field in expert_samples:
                            if isinstance(expert_samples[field], dict):
                                ...
                                if expert_samples[field].shape[1] > expert_samples[field].shape[0]:
                        ...
                        for field in gen_batch.keys():
                            ...
                            if isinstance(value, dict):
                                ...
                                if value.dtype == np.object_:
                                    ...
                                    gen_batch[field] = th.tensor(value, device=self.device).unsqueeze(0)  # Add dimension
                            elif isinstance(value, list):
                                # Handle lists, including potential dictionaries
                                gen_batch[field] = [
